'use strict'

const Emitter = require('events')

class Context extends Emitter {
	constructor(pipeline, fn) {
		super()
		this.exports = {}
		this.pipeline = pipeline
		this._fn = fn
		this._state = null
		this._dependencies = new Set()
		this._dependents = new Set()
		this._updateHandlers = new Map()
		this._disposed = false
	}

	get isEntry() {
		return this.pipeline._entry === this._fn
	}

	get isDisposed() {
		return this._disposed
	}

	get isUnused() {
		return !this.isEntry && this._dependents.size === 0
	}

	get disposeAfterPipeline() {
		return this.pipeline._disposeAfter.has(this)
	}

	set disposeAfterPipeline(value) {
		if (value) {
			this.pipeline._disposeAfter.add(this)
		} else {
			this.pipeline._disposeAfter.delete(this)
		}
	}

	_ensureDependency(fn) {
		const ctx = this.pipeline._ensureContext(fn)
		this._dependencies.add(ctx)
		ctx._dependents.add(this)
		return ctx
	}

	_start() {
		if (!this._state) {
			this._setState((async () => {
				await this.pipeline._disposalState(this._fn)
				return this._fn(this)
			})())
		}
		return this._state
	}

	_setState(state) {
		if (state instanceof Promise) {
			state.catch(() => {})
		} else {
			state = Promise.resolve(state)
		}
		this._state = state
		if (this.isEntry) {
			this._state.then(
				value => this.pipeline._onResolve(value),
				value => this.pipeline._onReject(value))
		}
	}

	_update(fn) {
		const handler = this._updateHandlers.get(fn)
		if (handler) {
			handler(this.use(fn))
		} else {
			this.dispose(false)
		}
	}

	use(fn) {
		if (fn instanceof Context) {
			fn = fn._fn
		}
		if (typeof fn !== 'function') {
			throw new TypeError('fn must be a function.')
		}
		if (this._disposed) {
			throw new Error('Context is disposed.')
		}
		return this._ensureDependency(fn)._start()
	}

	push(state) {
		this._setState(state)
		for (const dependent of this._dependents) {
			dependent._update(this._fn)
		}
	}

	pull(fn, handler) {
		if (fn instanceof Context) {
			fn = fn._fn
		}
		if (typeof fn !== 'function') {
			throw new TypeError('fn must be a function.')
		}
		if (typeof handler !== 'function') {
			throw new TypeError('handler must be a function.')
		}
		if (this._disposed) {
			return false
		}
		if (this._updateHandlers.has(fn)) {
			throw new Error('Multiple update handlers for the same entry.')
		}
		this._updateHandlers.set(fn, handler)
		this._ensureDependency(fn)
		return true
	}

	pullImmediate(fn, handler) {
		if (fn instanceof Context) {
			fn = fn._fn
		}
		if (typeof fn !== 'function') {
			throw new TypeError('fn must be a function.')
		}
		if (typeof handler !== 'function') {
			throw new TypeError('handler must be a function.')
		}
		if (this._disposed) {
			return false
		}
		if (this._updateHandlers.has(fn)) {
			throw new Error('Multiple update handlers for the same entry.')
		}
		this._updateHandlers.set(fn, handler)
		const ctx = this._ensureDependency(fn)
		setImmediate(() => handler(ctx._start()))
		return true
	}

	isPulling(fn) {
		if (fn instanceof Context) {
			fn = fn._fn
		}
		if (typeof fn !== 'function') {
			throw new TypeError('fn must be a function.')
		}
		return this._updateHandlers.has(fn)
	}

	isDependency(fn) {
		return this._dependencies.has(this.pipeline.getContext(fn))
	}

	isDependent(fn) {
		return this._dependents.has(this.pipeline.getContext(fn))
	}

	drop(fn) {
		if (fn instanceof Context) {
			fn = fn._fn
		}
		if (typeof fn !== 'function') {
			throw new TypeError('fn must be a context or a function.')
		}
		this._updateHandlers.delete(fn)
		const dependency = this.pipeline._contexts.get(fn)
		if (dependency) {
			dependency._dependents.delete(this)
			this._dependencies.delete(dependency)
		}
	}

	dispose(silent = false) {
		if (!this._disposed) {
			this._disposed = true

			const addDisposal = state => {
				deprecateAddDisposal()
				this.pipeline._addDisposal(this._fn, state)
			}
			for (const handler of this.listeners('dispose')) {
				this.pipeline._addDisposal(this._fn, handler(addDisposal))
			}

			this.pipeline._contexts.delete(this._fn)

			for (const dependency of this._dependencies) {
				dependency._dependents.delete(this)
				this._dependencies.delete(dependency)
			}
			for (const dependent of this._dependents) {
				dependent._dependencies.delete(this)
				this._dependents.delete(dependent)
				if (!silent) {
					dependent._update(this._fn)
				}
			}
			if (!silent && this.isEntry) {
				this.pipeline._onEntryDisposed()
			}
		}
	}
}

function deprecateAddDisposal() {
	if (!deprecateAddDisposal.done) {
		deprecateAddDisposal.done = true
		process.emitWarning('Using addDisposal(..) in context \'dispose\' event handlers is deprecated. Return a promise from the event handler instead.', 'DeprecationWarning')
	}
}

exports.default = Context
