...
class Context extends Emitter {
	...
	_setState(state) {
		...
		this._state = state
		if (this.isEntry) {
			this.pipeline._onSetEntryState(state, this)
		}
	}

	_update(ctx) {
		const handler = this._updateHandlers.get(ctx._fn)
		if (handler) {
			handler(this.use(ctx._fn), ctx) // TODO: Test this.
		} else {
			this.dispose(false)
		}
	}
	...
	dispose(silent = false) {
		if (!this._disposed) {
			...
			for (const handler of this.listeners('dispose')) {
				this.pipeline._addDisposal(this, handler())
			}
